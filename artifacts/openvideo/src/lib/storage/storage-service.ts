...
class StorageService {
  private projectsAdapter: IndexedDBAdapter<TProject>;
  private savedSoundsAdapter: IndexedDBAdapter<SavedSoundsData>;
  private config: StorageConfig;

  constructor() {
    this.config = {
      projectsDb: "bviral-video-editor-projects",
      mediaDb: "bviral-video-editor-media",
      timelineDb: "bviral-video-editor-timelines",
      savedSoundsDb: "bviral-video-editor-saved-sounds",
      version: 1,
    };

    this.projectsAdapter = new IndexedDBAdapter<TProject>(
  ...
  async saveProject({ project }: { project: TProject }): Promise<void> {
    await this.projectsAdapter.set(project.id, {
      ...project,
      updatedAt: new Date(),
    });
  }
  ...
  async loadProject({ id }: { id: string }): Promise<TProject | null> {
    try {
      const project = await this.projectsAdapter.get(id);
      return project
        ? {
            ...project,
  ...
  async loadAllProjects(): Promise<TProject[]> {
    try {
      ...
      const projects = await Promise.all(projectIds.map((id) => this.loadProject({ id })));

      return projects
        .filter((project): project is TProject => Boolean(project))
        .sort((a, b) => b.updatedAt.getTime() - a.updatedAt.getTime());
    } catch (e) {
      console.error("Error loading all projects from local storage:", e);
      return [];
    }
  }
