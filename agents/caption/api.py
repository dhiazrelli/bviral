...
def burn_ass_subtitles(video_path: str, ass_path: str, output_path: str):
    # ffmpeg's filtergraph parser treats '\' and ':' as special characters, so a
    # raw Windows path (outputs\tmpXXXX\subtitles.ass) gets mangled before libass
    # can open it. Use forward slashes and escape the drive colon. Let libass use
    # the OS font provider (directwrite on Windows, fontconfig on Linux) instead
    # of a hard-coded Linux fontsdir.
