...
# JSON sanitizing
# ---------------------------------------------------------------------------
# The pipeline produces NumPy scalars/arrays (SHAP impacts, predicted views,
# embeddings) that FastAPI's default encoder cannot serialize -> 500. Convert
# everything to native Python types before returning.
...
# Load .env files so GROQ_API_KEY (etc.) don't have to be injected into the
# shell every run. We check the local agent dir first, then the monorepo root
# (agents/ViralAgent -> agents -> repo root). override=False means a value
# already exported in the shell still wins.
try:
    from dotenv import load_dotenv  # python-dotenv

    REPO_ROOT = BASE_DIR.parent.parent
    for _env_path in (BASE_DIR / ".env", REPO_ROOT / ".env"):
        if _env_path.is_file():
            load_dotenv(_env_path, override=False)
except ModuleNotFoundError:
    logging.getLogger("viral_api.boot").warning(
        "python-dotenv not installed -- .env files will be ignored. "
        "Run: pip install python-dotenv"
    )

# GROQ_API_KEY must come from the environment (.env / shell). The LLM step is
# non-fatal (see /analyze), so a missing key degrades gracefully rather than
# crashing -- but ~60 of the 476 features come from it, so set it for accuracy.
if not os.environ.get("GROQ_API_KEY"):
    log_boot = logging.getLogger("viral_api.boot")
    log_boot.warning(
        "GROQ_API_KEY is not set -- LLM-derived features will be empty and "
        "predictions will be less accurate. Set it in the environment."
    )
...
# Override paths to the folder that actually holds the trained artifacts.
# best_model.pkl, model_meta.json AND the PCA pickles (hook_pca.pkl,
# full_pca.pkl, text_pca.pkl) all sit directly next to this api.py, so PCA_DIR
# must point at BASE_DIR -- not the training-time features/pca_reduced subdir,
# or apply_pca() would silently skip reduction and build a malformed vector.
config.MODEL_DIR   = str(BASE_DIR)
config.PCA_DIR     = str(BASE_DIR)
