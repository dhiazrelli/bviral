...
def extract_acoustic_features(wav_path: str) -> dict:
    ...
    # Tempo (librosa >=0.10 returns tempo as a 1-element array, not a scalar)
...
def run_llm_analysis(df: pd.DataFrame, whisper_index: pd.DataFrame) -> pd.DataFrame:
    ...
    # API key comes from the GROQ_API_KEY environment variable.
    api_key = os.environ.get("GROQ_API_KEY")
    if not api_key:
        log.warning("GROQ_API_KEY not set; skipping LLM analysis.")
        return pd.DataFrame()
